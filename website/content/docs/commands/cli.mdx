...
| `tokens analyze\|compare\|tail [<runId>] [--json] [--all] [--lines N]` | Разбор token-трассы run'а: `analyze` — totals/byStep/bySource/byVaultFile/byEngramType + warnings (без runId — самый свежий по mtime; `--all` — пул по всем runs); `compare <runA> <runB>` — per-step Δ с флагом роста >10%; `tail [--lines N]` — последние N записей (default 20). С `--json` — структурированный вывод |
