...
Slash-команды работают внутри Claude Code (VS Code или CLI). Устанавливаются в `.claude/skills/vault__*/SKILL.md` при `dev-workflow init` (по одной директории на slash).
...
Storage: `<vault>/snapshots/<name>/` directory copy + `manifest.json`. Excludes `snapshots/` recursive, `.edit-log.json`, `.profile-state`, `*.engram-trace.jsonl`. См. [CLI Reference / Snapshots](./cli) для underlying команд.
