---
name: vault:engram-stats
description: Engram timeline dashboard — aggregated activity across recent workflow runs (memory_search/store/judge counts, per-step distribution, recent runs). Thin slash wrapper around dev-workflow engram-stats CLI. Use when diagnosing engram learning loop health, spotting runs that stored memories without judging, or verifying recent traces.
allowed-tools: [Bash]
invocation: user
---
